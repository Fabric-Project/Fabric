...
public class Vector4Distance : Node
{
    enum Vector4Distance : String, CaseIterable
    {
        case Eucledian
        case Manhattan
        case Cosine
        
        func distance(vectorA:simd_float4, vectorB:simd_float4) -> Float
        {
            switch self
            {
            case .Eucledian:
    ...
    override public class func registerPorts(context: Context) -> [(name: String, port: Port)] {
        ...
        return ports +
        [
            ("inputMetricParam",   ParameterPort(parameter:StringParameter("Distance Metric", Vector4Distance.Eucledian.rawValue, Vector4Distance.allCases.map({ $0.rawValue }), .dropdown, "Distance calculation method"))),
            ("inputVectorAParam",   ParameterPort(parameter:Float4Parameter("Vector 4", .zero, .inputfield, "First vector for distance calculation"))),
            ("inputVectorBParam",   ParameterPort(parameter:Float4Parameter("Vector 4", .zero, .inputfield, "Second vector for distance calculation"))),
    ...
    public var inputVectorAParam:ParameterPort<simd_float4> { port(named: "inputVectorAParam") }
    public var inputVectorBParam:ParameterPort<simd_float4> { port(named: "inputVectorBParam") }
                                 ...
                                 commandBuffer: MTLCommandBuffer)
    {
        ...
        {
            if let metric:Vector4Distance = .init(rawValue: inputMetric)
            {
                self.metric = metric
            }
        }
