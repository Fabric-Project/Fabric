...
public class MakeVector2ArrayNode : Node
{
    ...
    override public class var nodeDescription: String { "Zips two Float arrays componentwise into a Vector 2 array. Output length matches the longest input; shorter inputs pad with their last element (so a single-element array acts as a constant for that component). Unconnected or empty inputs default to 0." }
    ...
    override public class func registerPorts(context: Context) -> [(name: String, port: Port)] {
        ...
        [
            ...
            ("outputArray", NodePort<ContiguousArray<simd_float2>>(name: "Vector 2 Array", kind: .Outlet, description: "Array of (X, Y) vectors")),
        ]
    }
    ...
    public var outputArray: NodePort<ContiguousArray<simd_float2>> { port(named: "outputArray") }

    public override func execute(context: GraphExecutionContext,
                                 renderPassDescriptor: MTLRenderPassDescriptor,
                                 commandBuffer: MTLCommandBuffer)
    {
        guard self.inputX.valueDidChange || self.inputY.valueDidChange else { return }

        let xIn = self.inputX.value
        let yIn = self.inputY.value

        let count = [xIn?.count ?? 0, yIn?.count ?? 0].max() ?? 0
        guard count > 0 else {
            self.outputArray.send(ContiguousArray<simd_float2>())
            return
        }
        ...
        var output = ContiguousArray<simd_float2>()
        output.reserveCapacity(count)
        for i in 0..<count {
            output.append(simd_float2(xs[i], ys[i]))
        }
