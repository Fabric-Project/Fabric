...
public class Vector3ArrayFromXYZNode : Node
{
    public override class var name: String { "Vector 3 Array From XYZ" }
    public override class var nodeType: Node.NodeType { .Parameter(parameterType: .Array) }
    override public class var nodeExecutionMode: Node.ExecutionMode { .Processor }
    override public class var nodeTimeMode: Node.TimeMode { .None }
    override public class var nodeDescription: String { "Zips the X, Y and Z Float arrays componentwise into a Vector 3 array. Output length matches the longest input; shorter inputs pad with their last element (so a single-element array acts as a constant for that component). Unconnected or empty inputs default to 0." }
