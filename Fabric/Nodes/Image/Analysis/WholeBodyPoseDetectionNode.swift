//
//  WholeBodyPoseDetectionNode.swift
//  Fabric
//
...
/// Detects a 133-point COCO-Wholebody pose (body + feet + face + hands, via
/// RTMW) within a caller-supplied region of interest in a single pass. Wire
/// a Region Detection node's output into Region of Interest — without one
/// this runs on the full frame, accurate only if a person already fills
/// most of it. Hand/face detail here is inherently lower fidelity than the
/// dedicated Hand/Face Pose Analysis nodes, since all three share one
/// person-level crop.
public class WholeBodyPoseDetectionNode: Node
{
    override public class var name: String { "Whole-Body Pose Detection" }
    override public class var nodeType: Node.NodeType { .Image(imageType: .Analysis) }
    override public class var nodeExecutionMode: Node.ExecutionMode { .Processor }
    override public class var nodeTimeMode: Node.TimeMode { .None }
    override public class var nodeDescription: String { "Detects a 133-point COCO-Wholebody pose (body, feet, face, hands) in a single pass (via RTMW) and outputs it in unit coordinates. Wire a Region Detection node into Region of Interest for accurate tracking." }
    ...
    override public class func registerPorts(context: Context) -> [(name: String, port: Port)]
    {
        ...
        return ports +
        [
            ("inputImage", NodePort<FabricImage>(name: "Image", kind: .Inlet, description: "Input image to analyze for a whole-body pose")),
            ...
            ("inputModelTier", ParameterPort(parameter: StringParameter("Model Tier", "Medium", ["Medium"], .dropdown, "RTMW model size — only Medium is published upstream today"))),

            ("outputKeypoints", NodePort<ContiguousArray<simd_float2>>(name: "Keypoints", kind: .Outlet, description: "All 133 COCO-Wholebody keypoints (body, feet, face, hands, in that order) in unit coordinates")),
            ...
            ("outputLeftFoot", NodePort<ContiguousArray<simd_float2>>(name: "Left Foot", kind: .Outlet, description: "Left big toe, small toe, heel in unit coordinates")),
            ("outputRightFoot", NodePort<ContiguousArray<simd_float2>>(name: "Right Foot", kind: .Outlet, description: "Right big toe, small toe, heel in unit coordinates")),
            ("outputFace", NodePort<ContiguousArray<simd_float2>>(name: "Face", kind: .Outlet, description: "All 68 face keypoints in unit coordinates (not split into sub-regions — use Face Pose Analysis for that)")),
            ("outputLeftHand", NodePort<ContiguousArray<simd_float2>>(name: "Left Hand", kind: .Outlet, description: "All 21 left-hand keypoints in unit coordinates")),
            ("outputRightHand", NodePort<ContiguousArray<simd_float2>>(name: "Right Hand", kind: .Outlet, description: "All 21 right-hand keypoints in unit coordinates")),
        ]
    }
    ...
    public var outputLeftFoot: NodePort<ContiguousArray<simd_float2>> { port(named: "outputLeftFoot") }
    public var outputRightFoot: NodePort<ContiguousArray<simd_float2>> { port(named: "outputRightFoot") }
    public var outputFace: NodePort<ContiguousArray<simd_float2>> { port(named: "outputFace") }
    public var outputLeftHand: NodePort<ContiguousArray<simd_float2>> { port(named: "outputLeftHand") }
    public var outputRightHand: NodePort<ContiguousArray<simd_float2>> { port(named: "outputRightHand") }

    private static let fullFrameRegion = simd_float4(0, 0, 1, 1)
    private static let keypointCount = 133

    private static let leftFootIndices = Array(RTMPoseKeypointSchema.cocoWholeBody133FeetRange.lowerBound..<(RTMPoseKeypointSchema.cocoWholeBody133FeetRange.lowerBound + 3))
    private static let rightFootIndices = Array((RTMPoseKeypointSchema.cocoWholeBody133FeetRange.lowerBound + 3)..<RTMPoseKeypointSchema.cocoWholeBody133FeetRange.upperBound)
    ...
    override public func execute(renderer: GraphRenderer, executionInfo: GraphExecutionInfo, renderPassDescriptor: MTLRenderPassDescriptor, commandBuffer: MTLCommandBuffer) throws
    {
        ...
        self.outputLeftFoot.send(self.unitPoints(at: Self.leftFootIndices, in: self.lastKeypoints, aspect: aspect))
        self.outputRightFoot.send(self.unitPoints(at: Self.rightFootIndices, in: self.lastKeypoints, aspect: aspect))
        self.outputFace.send(self.unitPoints(at: Array(RTMPoseKeypointSchema.cocoWholeBody133FaceRange), in: self.lastKeypoints, aspect: aspect))
        self.outputLeftHand.send(self.unitPoints(at: Array(RTMPoseKeypointSchema.cocoWholeBody133LeftHandRange), in: self.lastKeypoints, aspect: aspect))
        self.outputRightHand.send(self.unitPoints(at: Array(RTMPoseKeypointSchema.cocoWholeBody133RightHandRange), in: self.lastKeypoints, aspect: aspect))
    }
