...
//
// description: Applies a soft white diffusion glow
...
typedef struct {
    ...
    float blur;      // slider, 0.0, 20.0, 0.0, BlurRadius
} PostUniforms;
...
//                        input4 + input5 + input6 + input7 + input8) * 0.125h;
    ...
    half4 result = mix(origLuma, exposureResult, half(uniforms.diffusion * 3.0) * luma * contrast); //luma * contrast);
