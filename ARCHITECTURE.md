...
`.Float` port outputs from a node must be **finite**. Emitting `NaN` or `±Inf` from a `.Float` (or `.Float` array) outlet is a serious bug: it can crash the application.
...
* **Uninitialized inputs at load time** — during graph decode, an upstream port's value may briefly be `nil`. If your node substitutes `Float.nan` as a default in that case, you've armed the cycle.
