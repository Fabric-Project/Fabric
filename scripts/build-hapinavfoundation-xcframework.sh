...
PROJECT_PATH="${REPO_ROOT}/HapInAVFoundation/HapInAVFoundation.xcodeproj"
SCHEME="HapInAVFoundation"
CONFIGURATION="${CONFIGURATION:-Release}"
OUTPUT_DIR="${1:-${REPO_ROOT}/Frameworks}"
OUTPUT_XCFRAMEWORK="${OUTPUT_DIR}/HapInAVFoundation.xcframework"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/hapinavfoundation-xcframework.XXXXXX")"
trap 'rm -rf "${TMP_DIR}"' EXIT

DERIVED_DATA="${TMP_DIR}/derived-universal"

echo "Building HapInAVFoundation.framework (${CONFIGURATION}, arm64+x86_64)..."
xcodebuild \
...
UNIVERSAL_FRAMEWORK="${DERIVED_DATA}/Build/Products/${CONFIGURATION}/HapInAVFoundation.framework"
